...
class UI {
  ...
  displayControlsMenu() {
    add([sprite("forest-background"), scale(4)])
    add([
      text("Управление", { font: "Round", size: 50 }),
      area(),
      anchor("center"),
      pos(center().x, center().y - 200)
    ])
    ...
    controlPrompts.add([
      text("Прыжок", { font: "Round", size: 32 }),
      pos(-190, 100)
    ])
    controlPrompts.add([
      text("Движение", { font: "Round", size: 32 }),
      pos(10, 100)
    ])
  ...
  displayGameOverScreen() {
    ...
    add([
      text("Игра закончена!", { size: 50, font: "Round" }),
      area(),
      anchor("center"),
      pos(center())
    ])
  ...
  displayEndGameScreen() {
    ...
    add([rect(1280, 720), color(0, 0, 0)])
    add([
      text("Вы настоящий Сарыгин! Дальше только Испания", {
        size: 50,
        font: "Round"
      }),
      area(),
